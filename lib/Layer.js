...
Layer.prototype.parseCSS = function () {
    ...
    // Add the font name
    css.fontFamily.fontName = css.fontFamily.family.replace(' ', '') + '-' + css.fontFamily.variant;
...
Layer.prototype.getCSS = function () {
    ...
    if ('' !== addFont && -1 === this.structure.fonts.indexOf(addFont) && undefined !== fonts2[addFont]) {
...
Layer.prototype.getIntegration = function (elements, element) {
    var _this = this,
        html = '',
        content = '',
        attributes = '',
    ...
    this.siblings.forEach(function (sibling) {
        ...
        if (elementTypes.indexOf(sibling.convention.extension) !== -1) {
            ...
            var siblingIntegration = sibling.getIntegration(elements,sibling);

            content += '{{ '+sibling.convention.extension+' }}';

            var elem = siblingIntegration.elements;
