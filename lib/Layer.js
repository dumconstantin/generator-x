    ...
    if ('textLayer' === style.type) {
        ...
        (function () {
            ...
            var bounds = style._get('text.bounds'),
                ...
                rowsNo = style._get('text.paragraphStyleRange').length,
            ...
            if (undefined !== textBounds.bottom) {
                boxHeight = (boundsHeight - (textBounds.bottom - textBounds.top)) * 2 + boxBoundsHeight;
            } 
...
Layer.prototype.getIntegration = function(local) {
    ...
    if (!local)
        this.siblings.forEach(function (sibling) {
            //TODO see if is already used
            integration += sibling.getIntegration();
        });
