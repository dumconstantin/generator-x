...
function isIntersected(layer, list, minCount) {

    return minCount <= list.reduce(function (count, element) {
        if (
            ...
            (layer.css.top < element.css.bottom && layer.css.top <= element.css.top ||
                layer.css.bottom > element.css.top && layer.css.bottom <= layer.css.bottom)
        ) {
            console.log(count);
            count += 1;
        }
...
Relative.prototype.moveTextLayers = function moveTextLayers() {
    var _this;
    ...
    function move(layers) {
        ...
        lists = layers.reduce(function (list, layer) {

            if ('textLayer' === layer.type) {
                list.text.push(layer);
            } else {
                list.shape.push(layer);
            }
        ...
        lists.text.forEach(function (layer) {
            console.log(isIntersected(layer, lists.shape, 2));
            if (isIntersected(layer, lists.shape, 2)) {
                scheduled.push(layer);
            }
        });

        if (lists.text.length > 0 && lists.shape.length > 0) {
            layer = _this.structure.createLayer(lists.shape);
            layer.ignore = true;
            layer.cssId = 'created-' + layer.cssId;
        }

        /*
        console.log(lists.text.map(function (layer) {
            return layer.name;
        }));
        */
        ...
        lists.shape.forEach(function (layer) {
            move(layer.siblings);
        });
