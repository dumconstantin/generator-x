...
function detectComposedElements(elements, parentRow) {
    ...
    while (0 < toProcess.length) {
        ...
        if (1 < selectedElements.length) {
            ...
            selectedElements.forEach(function (element) {
                composedList.push(element.id);
            });

            selectedElements.forEach(function (element) {
                
                if (undefined !== element.next && -1 === composedList.indexOf(element.next.id)) {
                    if (undefined === next) {
                        next = element.next;
                    }

                    if (next.left > element.next.left) {
                        next = element.next;
                    }
                }

                if (undefined !== element.prev && -1 === composedList.indexOf(element.prev.id)) {
                    if (undefined === prev) {
                        prev = element.prev;
                    }

                    if (prev.right < element.next.right) {
                        prev = element.prev;
                    }
                }
    ...
    composedElements.forEach(function (composedElement) {
        composedElement.rows = processElements(composedElement.children, composedElement);
    });
