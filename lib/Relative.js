        ...
        for (i = 0; i < layers.length; i += 1) {
            ...
            if (
                ...
                right > layers[i].css.right && left < layers[i].css.right) 
                   ...
                   // console.log(layers[i].css.bottom, layers[i].css.top, bottom, top);
...
Relative.prototype.selectLayers = function (layers) {
    ...
    for (i = 0; i < layers.length; i += 1) {
      //  console.log(i, layers.length, layers[i].id, layers[i].css.top, top, bottom);
        if ((bottom > layers[i].css.top && top < layers[i].css.top) &&
            (left >= layers[i].css.right || right <= layers[i].css.left)
        ) {
     ...
     //   console.log(selected.map(function (layer) { return layer.id; })); 
     //   console.log('---');
...
Relative.prototype.setColumnStyles = function () {
    ...
    this.walk(this.relativeTree, null, function (node, index, parent, parentIndex, level) {
        ...
        if ('column' === node.nodeType) {
            ...
            console.log(node.layer.cssId, node.layer.css.left, node.layer.css.right, node.layer.css.width);
