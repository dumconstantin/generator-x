...
function detectRow(elements) {
    ...
    elements[topIndex].markToRemove = true;
    ...
    do {
        ...
        toProcess.every(function (elementIndex, index) {
            ...
            if (boundaryBottom >= center && boundaryTop <= center) {
                ...
                elements[elementIndex].markToRemove = true;
    ...
    rowElements.forEach(function (elementIndex) {
        elements[elementIndex].row = row;
        
        if (0 !== elements[elementIndex].children.length) {
            processElements(elements[elementIndex].siblings, row);
        }
    });

    elements.forEach(function (element, index) {
        if (true === element.markToRemove) {
        ...
        do {
            ...
            toProcess.every(function (elementIndex, toProcessIndex) {
                ...
                if (
                    ...
                    composedElements.push(elementIndex);
                    selectedElements.push(elements[elementIndex]);
...
function getRows(elements, parentRow) {
    var rows = [],
        rowIndex = 0,
        queuedElements = [];
    ...
    while (0 < queuedElements.length) {
        rows[rowIndex] = detectRow(queuedElements);
        rows[rowIndex].parent = parentRow;
 ...
 * @return undefined
 */
function processElements(elements, parentRow) {
    var boundries,
        rows;
