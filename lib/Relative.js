...
Relative.prototype.createColumn = function (layer, parent, layerIds) {
    ...
    column.layer.siblings.forEach(function (layer) {
        ...
        ['left', 'right'].forEach(function (pos) {
            ...
            if (undefined === store.vertical[layer.css[pos]]) {
                store.vertical[layer.css[pos]] = [];
                vertical[layer.css[pos]] = [];
            }

            store.vertical[layer.css[pos]].push(layer);
            vertical[layer.css[pos]].push([layer.css.top, layer.css.bottom]);

            if (-1 === v.indexOf(layer.css[pos])) {
                v.push(layer.css[pos]);
            }
        ...
        ['top', 'bottom'].forEach(function (pos) {
            ...
            if (undefined === store.horizontal[layer.css[pos]]) {
                store.horizontal[layer.css[pos]] = [];
                horizontal[layer.css[pos]] = [];
            }

            store.horizontal[layer.css[pos]].push(layer);
            horizontal[layer.css[pos]].push([layer.css.left, layer.css.right]);

            if (-1 === h.indexOf(layer.css[pos])) {
                h.push(layer.css[pos]);
            }
        });
    ...
    v.forEach(function (vVal) {
        ...
        store.vertical[vVal].forEach(function (layer) {
            ...
            console.log(vVal, layer.css.top, top, bottom, intersections);
        });
