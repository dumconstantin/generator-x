...
//         log(siblings, 'row finished');
    ...
    function logStructure(siblings, condition) {

        siblings.forEach(function (sibling) {

            if (undefined !== sibling.cssId && undefined !== condition && condition === sibling.cssId.substr(0, condition.length)) {
                console.log(sibling.cssId);
            } else {
                console.log(sibling.cssId);
            }

            if (0 !== sibling.children.length) {
                logStructure(sibling.children, condition);
            }

        });
    }

    function setStructure(children) {

        children.forEach(function (child, index) {

            ['top', 'left', 'bottom', 'right', 'width', 'height'].forEach(function (prop) {
                child[prop] = child.css[prop];
            });


            if (0 !== child.siblings.length) {
                setStructure(child.siblings);
            }

            child.children = child.siblings;
    ...
    setStructure(this.parent.siblings);

    var result = Relative(this.parent.siblings);


   // return;
    // console.log(result[0].children[0].children);
    this.parent.siblings = createNewNodes(result);
