...
Structure.prototype.getIntegration = function() {
    ...
    console.log('Am ajuns la integrare MĂ!')

    var root = new DefaultPlugin(this.parent);
    var integration = root.getIntegration();

    console.log(JSON.stringify(integration,null,"\t"));
...
Structure.prototype.outputCode = function () {
    ...
    this.fonts.forEach(function (fontName) {
        ...
        fontExtensions.forEach(function (extension) {
            var fontFile = path.resolve(__dirname, '../fonts/') + '/'
                        + fonts2[fontName]
                        + '.' + extension,
                stream;

            if (true === fs.existsSync(fontFile)) {

                _this.fontsToTransfer.push(fontName);

                stream = fs.createWriteStream(_this.folders.fonts + fontName + '.' + extension);
                ...
                fs.createReadStream(fontFile).pipe(stream);
            } else {
                // The file does not exists.
                // TODO: Make sure all extensions are included/installed.
            }
        });
...
Structure.prototype.finishedFont = function (fontName) {
    var fontIndex = _this.fontsToTransfer.indexOf(fontName)
    if (-1 !== fontIndex) {
        _this.fontsToTransfer.splice(fontIndex, 1);

        if (0 === _this.fontsToTransfer.length) {
            this.events.emit('finishedFontTransfer')
        }
...
Structure.prototype.optimiseCode = function () {
    ...
    /*
    var inheritable = ['azimuth', 'border-collapse', 'border-spacing', 
    ...
    function removeInheritable(elements) {
        elements.forEach(function (element) {
            Object.keys(element.parsedCss).forEach(function (cssProperty) {
                if (-1 !== inheritable.indexOf(cssProperty)) {
            ...
            removeInheritable(element);
        });

    }

    removeInheritable(this.parent.siblings);
...
Structure.prototype.optimiseCode2 = function () {
