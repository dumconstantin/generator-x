...
//         log(siblings, 'row finished');
    ...
    function removeAdvancedAbsolute(siblings) {
        var rows = {},
            oneOffs = [],
            normalization = 5;
        ...
        siblings.forEach(function (sibling) {
            ...
            if (undefined === rows[sibling.css.top]) {
                rows[sibling.css.top] = [];
            }

            rows[sibling.css.top].push(sibling);
        ...
        Object.keys(rows).forEach(function (row) {
            ...
            if (1 === rows[row].length) {
                oneOffs.push(rows[row][0]);
                return;
            }
            ...
            rows[row].every(function (element, index, elements) {
                var distance;
                ...
                distance = element.initialCss.left - (elements[index - 1].initialCss.left + elements[index - 1].initialCss.width);
                
                if (undefined === firstMargin) {
                    firstMargin = distance;

                    elements[index - 1].parent.css.fontSize = 0;

                    elements[index - 1].css.position = 'relative';
                    elements[index - 1].css.top = 0;
                    elements[index - 1].css.left = 0;
                    elements[index - 1].css.marginRight = firstMargin;
                    elements[index - 1].css.display = 'inline-block';
                }

                console.log(element.name + ' vs ' + distance + ' vs ' + firstMargin);

                if (firstMargin + normalization > distance 
                    && firstMargin - normalization < distance
                ) {
                    ...
                    if (index < elements.length - 1) {
                        element.css.marginRight = firstMargin;
                    }
        ...
        siblings.forEach(function (sibling) {
            removeAdvancedAbsolute(sibling.siblings);
        });
    }
    ...
    removeAdvancedAbsolute(this.parent.siblings);
