...
//         log(siblings, 'row finished');
    ...
    function orderByColumn(siblings) {
        ...
        siblings.forEach(function (sibling) {
            orderByColumn(sibling.siblings);
        });
    ...
    function removeAdvancedAbsolute(parent) {
        var left = 0,
            rows = [],
            elementsNo = 0,
            cols = 0,
        ...
        // Generate an array with only the positional values of each sibling.
        parent.siblings.forEach(function (sibling, index) {
            var top = sibling.css.top - sibling.parent.css.top - sibling.parent.css.paddingTop,
                left = sibling.css.left - sibling.parent.css.left - sibling.parent.css.paddingLeft;

            cells.push({
                y: top + sibling.css.height / 2,
                x: left + sibling.css.width / 2,
                w: sibling.css.width,
                h: sibling.css.height,
                bottomGap: 0,
                leftGap: 0,
                inlineBlockAdjustment: 0,
                top: sibling.css.top - sibling.parent.css.top,
                left: sibling.css.left - sibling.parent.css.left,
                element: sibling
            });
        ...
        // For each element find all other elements that have 
        // the center in between the height of that element.
        for (i = 0; i < cells.length; i += 1) {
            
            for (j = 0; j < cells.length; j += 1) {
            
                is.upperBounds = cells[j].y >= cells[i].y - cells[i].h / 2;
                is.lowerBounds = cells[j].y <= cells[i].y + cells[i].w / 2;

                if (is.upperBounds && is.lowerBounds) {
                    connections[j] = i;
                }
        ...
        // Create an array with the elements organised on the 
        // ids from the connection generation.
        var map = {},
            inc = 0;

        for (i = 0; i < connections.length; i += 1) {

            if (false === map.hasOwnProperty(connections[i])) {
                map[connections[i]] = inc;
                rows[map[connections[i]]] = [];
                inc += 1;
            }
            elementsNo += 1;
            rows[map[connections[i]]].push(cells[i]);

            if (cols < rows[map[connections[i]]].length) {
                cols = rows[map[connections[i]]].length;
            }
        }
        


        // Create the relationship between elements and remove the absolute 
        // positioning.
        if (2 <= elementsNo) {

            var cellWidth = parent.css.width / cols,
                cellHeight = parent.css.height / rows.length;
            ...
            rows.forEach(function (col, index) {
                ...
                col.forEach(function (cell) {
                    var top = Math.floor(cell.x / cellWidth),
                        left = Math.floor(cell.y / cellHeight);

                    rowWidth += cell.element.css.width;

                    if (rowHeight < cell.h) {
                        rowHeight = cell.h;
                    }

                    cells[left][top].elements.push(cell);
                });

                if (rowsWidth < rowWidth) {
                    rowsWidth = rowWidth;
                }
            ...
            var marginRight = (parent.css.width - rowsWidth) / (cols - 1),  
                marginBottom = (parent.css.height - rowsHeight) / (rows.length - 1),
                leftBoundry = parent.css.left + parent.css.paddingLeft,
                topBoundry = parent.css.top + parent.css.paddingTop;

            parent.css.fontSize = 0;

            // Set the new relative/inline-block values for
            cells.forEach(function (row, rowIndex, rows) {
                row.forEach(function (col, colIndex, cols) {
                    var cell = col.elements[0],
                        prev,
                        prevRowHeight = 0,
                        prevLeftGap = 0,
                        prevBottomGap = 0,
                        prevH = 0,
                        prevW = 0,
                        prevInlineAdjustment = 0,
                        cellRightMargin = marginRight,
                        topDiff = 0,
                        nextIndex = colIndex,
                        leftDiff = 0;
                    ...
                    cell.element.css.display = 'inline-block';
                    cell.element.css.verticalAlign = 'top';


                    // Find how many cells on this row are empty
                    while(undefined !== cols[nextIndex + 1] && undefined === cols[nextIndex + 1].elements[0]) {
                        nextIndex += 1;
                    }
                    ...
                    if (nextIndex !== colIndex) {
                        ...
                        // Find if the last empty cell is actually the last cell in the row
                        if (undefined === cols[nextIndex + 1]) {
                            rightPos = cellWidth * nextIndex + cell.element.css.right + (cellWidth - cell.w);
                        } else {
                            rightPos = cols[nextIndex + 1].elements[0].element.css.left;
                        }

                        cellRightMargin = rightPos - cell.element.css.right;
                    }

                    // topDiff and leftDiff are created because the cell will not
                    // have it's center always relevant. 
                    // Sometimes the element will not be centered in the cell but have
                    // a fraction above or bellow the center as dictated by the margins.
                    // Example:
                    // cellHeight = 20
                    // elementHeight = 13
                    // margin = 15
                    // The first element will have a 15 bottom margin but inside the cell there
                    // will be 20 - 13 (cellHeight - elementHeight) = 7px worth of margin. The rest
                    // of the 15 - 7 (margin - diff) = 8px will be sent to the next cell.
                    // The bottom cell element will have to start in the normal flow at 8px bellow
                    // the cell's top.

                    if (0 !== colIndex) {
                        prev = cells[rowIndex][colIndex - 1].elements[0];

                        if (undefined !== prev) {
                            prevW = prev.element.css.width;
                            prevLeftGap = prev.leftGap;
                            cell.leftGap = cellRightMargin - (cellWidth - (prevW + prevLeftGap));
                        } else {
                            cell.leftGap = cellWidth - cell.w;
                        }

                        leftDiff = colIndex * cellWidth + cell.leftGap;
                    }

                    cell.element.css.left = cell.element.css.left - leftBoundry - leftDiff;

                    if (0 !== rowIndex) {

                        cells[rowIndex - 1].forEach(function (cell) {
                            
                            if (undefined === cell.elements[0]) {
                                return;
                            }

                            if (prevRowHeight < cell.elements[0].h) {
                                prevRowHeight = cell.elements[0].h;
                            }
                        });

                        prev = cells[rowIndex - 1][colIndex].elements[0];

                        if (undefined !== prev) {
                            prevH = prev.element.css.height;
                            prevBottomGap = prev.bottomGap;
                            prevInlineAdjustment = prev.inlineBlockAdjustment;
                        }

                        // Calculate the remaining bottom gap that is possible given the top gap from
                        // the previous element.
                        cell.bottomGap = marginBottom - (cellHeight - (prevH + prevBottomGap));

                        // Inlineblock elements are aligned to the top. If an element has a different
                        // height than the ones next to it, it will push the next row down.
                        cell.inlineBlockAdjustment = prevRowHeight - prevH + prevInlineAdjustment;

                        topDiff = rowIndex * cellHeight + cell.bottomGap + cell.inlineBlockAdjustment;
                    }

                    cell.element.css.top = cell.element.css.top - topBoundry - topDiff;
                    
                    if (colIndex < cols.length - 1) {
                        cell.element.css.marginRight = Math.floor(cellRightMargin);
                    }

                    if (rowIndex < rows.length - 1) {
                        cell.element.css.marginBottom = Math.floor(marginBottom);
                    }
            ...
            // Reorder the elements according to the new order.
            cells.forEach(function (row, rowIndex, rows) {
                row.forEach(function (col, colIndex, cols) {
                    var elementIndex,
                        element;

                    if (undefined === col.elements[0]) {
                        return;
                    }

                    element = col.elements[0].element

                    parent.siblings.every(function (sibling, index) {
                        if (sibling.id === element.id) {
                            elementIndex = index;
                            return false;
                        } else {
                    ...
                    parent.siblings.splice(elementIndex, 1);

                    parent.siblings = parent.siblings.concat([element]);

                });
            });

        } else {
            // The element might be compatible with centering.
        }

        parent.siblings.forEach(function (sibling) {
            if (0 !== sibling.length) {
                removeAdvancedAbsolute(sibling);
            }
        });
    ...
    setPaddings(this.parent);

    // removeSimpleAbsolute(this.parent.siblings);
    removeAdvancedAbsolute(this.parent);
