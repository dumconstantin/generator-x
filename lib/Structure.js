...
Structure.prototype.refreshCode = function () {
    ...
    this.parent.siblings.forEach(function (layer) {
        layer.generateHTML().generateCSS();
    });
...
Structure.prototype.copyFonts = function () {
    ...
    this.fonts.forEach(function (fontName) {
        ...
        fontExtensions.forEach(function (extension) {
            var fontFile = fonts2[fontName] + '.' + extension;
        ...
        if (0 !== scheduledFontFiles.length) {
            ...
            _this.css += getCSSFontFamily(fontName);
        }
...
Structure.prototype.optimiseCode = function () {
    ...
    function parseCssPatterns() {
        ...
        // Remove all properties that have only one element
        // because these cannot create a pattern.
        
        Object.keys(cssPatterns).forEach(function (cssProperty) {

            Object.keys(cssPatterns[cssProperty]).forEach(function cssValue) {
                if (1 === cssPatterns[cssProperty][cssValue]) {
    ...
    // Find the bottom most left most layer
    var sibling = this.parent.siblings[0];
    if (0 !== sibling.siblings.length) {
        sibling = sibling.siblings[0];
    }

    function walkLayerTree(sibling, parseFunction) {

        parseFunction(sibling);

        if (undefined !== sibling.next) {
            ...
            if (undefined !== sibling.parent) {
                sibling = sibling.parent;
            }
        }

        // We exit when we get back again at the top most layer.
        if (undefined !== sibling.parent) {
            walkLayerTree(sibling, parseFunction);   
        } else {
            return;
        }
    }

    walkLayerTree(sibling, removeInheritable);
    walkLayerTree(sibling, removeDefaultValues);
    walkLayerTree(sibling, extractCssPatterns);
